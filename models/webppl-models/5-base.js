...
// webppl 5-base.js --require webppl-viz underscore
...
// alternative is to say: we know that there are things called "green" by a speaker, while we don't know that there are things a speaker finds "beautiful"
...
var distJudgmentStochasticObjective = Infer(
  ...
  model() { var judgmentObj =  flip(0.2) ? posterior_judgment_without_object(0,1,chosenObject) : posterior_judgment(0,1,chosenObject);
            var judgmentObjOther =  flip(0.2) ? posterior_judgment_without_object(1,1,chosenObject) : posterior_judgment(1,1,chosenObject);

            var judgmentSubj = flip(0) ? posterior_judgment_without_object(0,0,chosenObject) : posterior_judgment(0,0,chosenObject);
            var judgmentSubjOther = flip(0) ? posterior_judgment_without_object(1,0,chosenObject) : posterior_judgment(1,0,chosenObject);

            var affect = affect_meaning(0)
            return [judgmentSubj,judgmentSubjOther,judgmentObj,judgmentObjOther, affect]}});
...
var distJudgmentStochasticSubjective = Infer(
  {method: 'rejection',samples:500,
  model() { var judgmentSubj = flip(0.2) ? posterior_judgment_without_object(0,0,chosenObject) : posterior_judgment(0,0,chosenObject);
            var judgmentSubjOther = flip(0.2) ? posterior_judgment_without_object(1,0,chosenObject) : posterior_judgment(1,0,chosenObject);

            var judgmentObj =  flip(0) ? posterior_judgment_without_object(0,1,chosenObject) : posterior_judgment(0,1,chosenObject);
            var judgmentObjOther =  flip(0) ? posterior_judgment_without_object(1,1,chosenObject) : posterior_judgment(1,1,chosenObject);
...
var gainObjective =  (entropy(distJudgmentStochasticObjective))
var gainSubjective = (entropy(distJudgmentStochasticSubjective))

//console.log("ENTROPY")
//console.log(entropy(distJudgmentStochastic))
console.log("ENTROPY (OBJECTIVE FIRST)  "+gainObjective)
console.log("ENTROPY (SUBJECTIVE FIRST) "+gainSubjective)
