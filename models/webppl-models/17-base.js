// webppl 17-base.js   --require webppl-viz underscore
...
var prior_adj = function(adjective, agreementTable) {
   return function(x) {    
                           var j1 = flip(C[adjective]) // A(s1,x)
                           var j2 = agreementTable[adjective] * (j1 ? 1 : 0) + (1-agreementTable[adjective]) * C[adjective]; // A(s2,x)
                           return [j1, flip(j2)] };
}
...
var world_prior = function(agreementTable) {
 return [map(prior_adj(0,agreementTable), _.range(n_obj)),map(prior_adj(1,agreementTable), _.range(n_obj)),map(prior_adj(2,agreementTable), _.range(n_obj)),map(prior_adj(3,agreementTable), _.range(n_obj))];
}
...
var third = cache(function(prefix, lossProb2, lossProb1, agreementTable) {
   Infer({method : 'rejection', samples:100, incremental:true,
      ...
      var corruption3 = corrupt([prefix[0], prefix[1], prefix[2]])
...
var posteriorRestrictionToObjectsAndAdjectivesForSent = cache(function(sentence, agreement1, agreement2, lossProb2, lossProb1) {
     Infer({method: 'rejection', samples : 10, incremental:true,
            model() {
               var agreementTable = sample(generateAgreementTable(agreement1, agreement2))
               var model = sample(third(sentence, lossProb2, lossProb1, agreementTable))
...
//console.log(restrictionToObjectsAndAdjectivesForSent(options[1]))
   ...
   var speaker = cache(function(rationality, agreement1, agreement2, lossProb2, lossProb1) {
                      var speakerModel = Infer({method : 'enumerate',
                          model() {
                          var sentence = options[sample(RandomInteger({n : 2}))];
                          factor(-rationality*entropy(posteriorRestrictionToObjectsAndAdjectivesForSent(sentence, agreement1, agreement2, lossProb2, lossProb1)))
                          return sentence;
                       }});
                       return Gaussian({mu : speakerModel.getDist()['[0,1,1]'].prob, sigma : 0.1})
                     })
...
var computeOrderPreferences = Infer({method : 'MCMC', samples : 2000, model() { 
   var rationality = 10; //sample(Gaussian({mu : 0, sigma : 10}))
...
viz.hist(computeOrderPreferences)
