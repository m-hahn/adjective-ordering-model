...
import pyro
from pyro.distributions import Normal, Bernoulli, Gamma, Uniform
from pyro.infer import SVI
from pyro.optim import Adam
...
print "\t".join(map(str,["Loss3", "Alpha1", "Alpha2", "C", "UtilityDiff", "CostDiff", "Prob", "kappa1", "kappa2"])) #alpha1.data.numpy()[0], alpha2.data.numpy()[0], C.data.numpy()[0], (utilityLoss12-utilityLoss21).data.numpy()[0], (costLoss12-costLoss21),  probability.data.numpy()[0], (agreement[adj1] - agreement[adj2])]))


def model(dataset):
  #alpha1 = Variable(torch.FloatTensor([0.0]), requires_grad=True)
  alpha1 = pyro.sample("a1", Gamma(Variable(torch.FloatTensor([5.0])),Variable(torch.FloatTensor([1.0]))))
  #alpha2 = Variable(torch.FloatTensor([0.0]), requires_grad=True)
  alpha2 = pyro.sample("a2", Normal(Variable(torch.FloatTensor([5.0])), Variable(torch.FloatTensor([1.0]))))

  loss = pyro.sample("l", Uniform(Variable(torch.FloatTensor([0.0])), Variable(torch.FloatTensor([1.0]))))


  C_logit = pyro.sample("Cl", Normal(Variable(torch.FloatTensor([0.0])),Variable(torch.FloatTensor([0.5])))) #nn.Sigmoid()(C_logit)
  C = nn.Sigmoid()(C_logit)

  for i in range(n_adj):
    agreement[i] = random()
  while agreement[0] >= agreement[1]:
     agreement[0] = random()
     agreement[1] = random()
  ...
  if True:
     ...
     pmi1 = 0
     pmi2 = 0
#     pmi1 = pyro.sample(Gamma(5.0,1.0))
 #    pmi2 = pyro.sample(Gamma(5.0,1.0))
     adj1 = 0
     adj2 = 1
     ...
     logit = alpha1 * loss * (utilityLoss12 - utilityLoss21) + alpha2 * loss * (costLoss12 - costLoss21)
...
#     result = pyro.sample("result_"+str(i),  Bernoulli(logits=logit))
     print "\t".join(map(str,[loss.data.numpy()[0],alpha1.data.numpy()[0], alpha2.data.numpy()[0], C.data.numpy()[0], (utilityLoss12-utilityLoss21).data.numpy()[0], (costLoss12-costLoss21),  probability.data.numpy()[0], (agreement[adj1]), (agreement[adj2])]))
