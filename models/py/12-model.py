...
def model(dataset):
  ...
  alpha1 = Variable(torch.FloatTensor([ 5.071]))
  #alpha2 = Variable(torch.FloatTensor([0.0]), requires_grad=True)
  alpha2 = Variable(torch.FloatTensor([-0.390]))

  C_logit = Variable(torch.FloatTensor([-2.253]))
...
#     result = pyro.sample("result_"+str(i),  Bernoulli(logits=logit))
     print "\t".join(map(str,[point["Adj1"], point["Adj2"], (utilityLoss12-utilityLoss21).data.numpy()[0], (costLoss12-costLoss21), logit.data.numpy()[0], (agreement[adj1] - agreement[adj2])]))
